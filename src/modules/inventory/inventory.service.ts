...
import {
  Vehicle,
  VehicleDocument,
} from "../../database/schemas/vehicle.schema";
import {
  Rooftop,
  RooftopDocument,
} from "../../database/schemas/rooftop.schema";
import {
  ActionItem,
  ActionItemDocument,
} from "../../database/schemas/action.schema";
import {
  FeedLog,
  FeedLogDocument,
} from "../../database/schemas/feed-log.schema";
...
export class InventoryService {
  ...
  async getFeedStatus() {
    const latestFeed = (await this.feedLogModel
      .findOne()
      .sort({ createdAt: -1 })
      .lean()) as any;
    return {
  ...
  async triggerFeedReconciliation() {
    const { pentanaRecords, websiteRecords } =
      this.reconciliationService.parseAllCsvFiles();
  ...
  // ==========================================
  async getGroupOverview(page = 1, limit = 5) {
    const vehicles = await this.vehicleModel
      .find({ status: { $nin: EXITED_STATUSES } })
      .lean();
    const rooftops = await this.rooftopModel.find().lean();
    let actions = await this.actionItemModel.find({ status: "open" }).lean();
      ...
      const actionVehicles = vehicles
        .filter(
          (v) =>
            ...
            v.recommendedAction !== "HOLD",
        )
        .sort(
          (a, b) =>
            b.daysInStock * b.totalStockCost - a.daysInStock * a.totalStockCost,
        );
    ...
    const totalStockCost = vehicles.reduce(
      (sum, v) => sum + (v.totalStockCost || 0),
      0,
    );
    const floorplanExposure = vehicles.reduce(
      (sum, v) => sum + (v.floorplanExposure || 0),
      0,
    );
    const frontlineReadyUnits = vehicles.filter((v) => v.frontlineReady).length;
    const potentialGrossTotal = vehicles.reduce(
      (sum, v) => sum + (v.potentialGross || 0),
      0,
    );

    const aged60Units = vehicles.filter((v) => v.daysInStock >= 60);
    const aged60Cost = aged60Units.reduce(
      (sum, v) => sum + (v.totalStockCost || 0),
      0,
    );

    const aged90Units = vehicles.filter((v) => v.daysInStock >= 90);
    const aged90Cost = aged90Units.reduce(
      (sum, v) => sum + (v.totalStockCost || 0),
      0,
    );
    ...
    const rooftopStats = rooftops.map((r) => {
      ...
      const lotCost = lotVehicles.reduce(
        (sum, v) => sum + (v.totalStockCost || 0),
        0,
      );
      ...
      // Aging breakdown
      const bucket0_30 = lotVehicles.filter(
        (v) => v.agingBucket === "0-30",
      ).length;
      const bucket31_45 = lotVehicles.filter(
        (v) => v.agingBucket === "31-45",
      ).length;
      const bucket46_60 = lotVehicles.filter(
        (v) => v.agingBucket === "46-60",
      ).length;
      const bucket61_90 = lotVehicles.filter(
        (v) => v.agingBucket === "61-90",
      ).length;
      const bucket90Plus = lotVehicles.filter(
        (v) => v.agingBucket === "90+",
      ).length;
      ...
      return {
        ...
        aged60Percent:
          lotVehicles.length > 0
            ? Math.round((lotAged60 / lotVehicles.length) * 100)
            : 0,
    ...
    // Brand mix
    const brandMix = vehicles.reduce(
      (acc, v) => {
    ...
    const soldCount = await this.vehicleModel.countDocuments({
      status: "SOLD",
    });
    const dealPendCount = vehicles.filter(
      (v) => v.status === "DEAL PEND",
    ).length;
    ...
    const actionPage = Number(page) || 1;
    const actionLimit = Number(limit) || 5;
    ...
    return {
      kpiStrip: {
        ...
        frontlinePercent:
          totalUnits > 0
            ? Math.round((frontlineReadyUnits / totalUnits) * 100)
            : 0,
        potentialGrossTotal,
        ...
        deltas: {
          ...
          wholesaleExits: vehicles.filter((v) => v.status === "WHOLESALE")
            .length,
          transfers: 0,
      ...
      categoryMix,
      brandMix,
      groupActionQueue: paginatedActions,
      groupActionQueuePagination: {
        total: actionTotal,
        page: actionPage,
        limit: actionLimit,
        totalPages: Math.ceil(actionTotal / actionLimit),
      },
    };
  }
  ...
  // ==========================================
  async getDealerPrincipalCluster(
    clusterId: string = "cluster-hyundai-metro",
    page = 1,
    limit = 5,
  ) {
    ...
    const clusterActions = await this.actionItemModel
      .find({
        rooftopId: { $in: rooftopIds },
        status: "open",
      })
    ...
    const watchlist = allWatchlist
      .slice(
        ((Number(page) || 1) - 1) * (Number(limit) || 5),
        (Number(page) || 1) * (Number(limit) || 5),
      )
      .map((v) => ({
    ...
    const comparisonTable = rooftops.map((r) => {
      ...
      const reconCount = lotVehicles.filter((v) =>
        ["IN SERVICE", "RECO"].includes(v.status),
      ).length;
      const avgDis =
        lotVehicles.length > 0
          ? Math.round(
      ...
      return {
        ...
        aged45Percent:
          lotVehicles.length > 0
            ? Math.round((aged45Count / lotVehicles.length) * 100)
            : 0,
        reconPending: reconCount,
        potentialGross: lotVehicles.reduce(
          (sum, v) => sum + (v.potentialGross || 0),
          0,
        ),
    ...
    const agingWaterfall = [
      {
        bucket: "0-30 days",
        count: vehicles.filter((v) => v.agingBucket === "0-30").length,
        cost: vehicles
          .filter((v) => v.agingBucket === "0-30")
          .reduce((s, v) => s + v.totalStockCost, 0),
      },
      {
        bucket: "31-45 days",
        count: vehicles.filter((v) => v.agingBucket === "31-45").length,
        cost: vehicles
          .filter((v) => v.agingBucket === "31-45")
          .reduce((s, v) => s + v.totalStockCost, 0),
      },
      {
        bucket: "46-60 days",
        count: vehicles.filter((v) => v.agingBucket === "46-60").length,
        cost: vehicles
          .filter((v) => v.agingBucket === "46-60")
          .reduce((s, v) => s + v.totalStockCost, 0),
      },
      {
        bucket: "61-90 days",
        count: vehicles.filter((v) => v.agingBucket === "61-90").length,
        cost: vehicles
          .filter((v) => v.agingBucket === "61-90")
          .reduce((s, v) => s + v.totalStockCost, 0),
      },
      {
        bucket: "90+ days",
        count: vehicles.filter((v) => v.agingBucket === "90+").length,
        cost: vehicles
          .filter((v) => v.agingBucket === "90+")
          .reduce((s, v) => s + v.totalStockCost, 0),
      },
    ];
    ...
    return {
      ...
      kpiStrip: {
        ...
        demoUnits: vehicles.filter((v) =>
          ["DEMO", "LOANER", "DRIVE CARS"].includes(v.status),
        ).length,
        avgDis:
          vehicles.length > 0
            ? Math.round(
      ...
      watchlistPagination: {
        ...
        totalPages: Math.ceil(allWatchlist.length / (Number(limit) || 5)),
      },
  ...
  // ==========================================
  async getGeneralManagerRooftop(
    rooftopId: string = "booran-hyundai-berwick",
    category = "All",
    page = 1,
    limit = 10,
  ) {
    ...
    const pipeline = {
      incoming: vehicles.filter((v) =>
        ["IN-TRANSIT", "ON-ORDER"].includes(v.status),
      ).length,
      inRecon: vehicles.filter((v) => ["IN SERVICE", "RECO"].includes(v.status))
        .length,
      frontlineReady: vehicles.filter((v) => v.frontlineReady).length,
      reserved: vehicles.filter((v) => v.status === "DEAL PEND").length,
      soldThisWeek: await this.vehicleModel.countDocuments({
        rooftopId,
        status: "SOLD",
      }),
      demo: vehicles.filter((v) =>
        ["DEMO", "LOANER", "DRIVE CARS"].includes(v.status),
      ).length,
    ...
    const missingPhotos = vehicles.filter(
      (v) => !v.heroPhoto || v.photos.length === 0,
    );
    const missingAdvertisedPrice = vehicles.filter(
      (v) => v.advertisedPrice === null || v.advertisedPrice <= 0,
    );
    const aged90Units = vehicles.filter((v) => v.daysInStock >= 90);
    const reconBreaches = vehicles.filter(
      (v) =>
        ["IN SERVICE", "RECO"].includes(v.status) &&
    ...
    const inventoryPage = Number(page) || 1;
    const inventoryLimit = Number(limit) || 10;
    ...
    return {
      ...
      inventoryList: inventory.slice(
        (inventoryPage - 1) * inventoryLimit,
      ...
      inventoryPagination: {
        ...
        totalPages: Math.ceil(inventory.length / inventoryLimit),
      },
    };
  }
  ...
  async getUsedCarWorkbench(query: {
    ...
    const priceReviewCount = await this.vehicleModel.countDocuments({
      isPriceReviewRequired: true,
      status: { $nin: EXITED_STATUSES },
    });
    const missingPhotosCount = await this.vehicleModel.countDocuments({
      heroPhoto: "",
      status: { $nin: EXITED_STATUSES },
    });
    const wholesaleQueueCount = await this.vehicleModel.countDocuments({
  ...
  async getUnitDetails(identifier: string) {
    ...
    let vehicle = await this.vehicleModel
      .findOne({ stockNumber: identifier })
      .lean();
    if (!vehicle) {
    ...
    if (!vehicle) {
      throw new NotFoundException(
        `Vehicle with identifier ${identifier} not found`,
      );
    }

    // Sister units across other Booran rooftops (same model)
    const sisterUnits = await this.vehicleModel
      .find({
        model: vehicle.model,
